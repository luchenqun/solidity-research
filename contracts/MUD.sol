...
contract MetaUserDAOToken is ERC20, ERC20Burnable, Ownable {

    address immutable private creator;

    event mainnetmappingfreezed(string mainnetMappingFreezed);

    constructor() ERC20("MetaUserDAOMUD", "MUDT") {
        _frozen = false;
        creator = msg.sender;

        _mint(msg.sender, 10000e18);//977061247291058);//the real number will be set after eth mainnet token freezed for mapping
    }
    ...
    function mainNetMappingFreeze() external {
        ...
        _frozen = true;
        emit mainnetmappingfreezed("Freezed for mainnet mapping!");
    }
